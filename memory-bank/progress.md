...
## Completed
- Lighthouse audit (2026-09-17): Chrome before/after for backoffice Overview (desktop Performance 51→53, mobile 40→45). Public site CLI before/after in `audit/` (home mobile SEO 91→100, LCP 2.13s→1.75s). `useProtectedSession` paints the workspace before `/auth/me`. Single `MetricCard` / `getOperationsSnapshot`; `BrandMark` / `VisitNotice`; `robots.ts`.
