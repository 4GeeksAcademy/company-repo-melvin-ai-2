...
- **`backoffice`** — internal operations overview plus **Supplier directory** at `/suppliers`. Local: `cd uis/backoffice && npm run dev` (port **3101**). Docker Compose: port **3001** (`docker compose up --build` from the repo root). Browser API: `NEXT_PUBLIC_API_BASE_URL=http://localhost:8000`. Container API: `INTERNAL_API_URL=http://backend:8000`. CONTEXT: [`memory-bank/supplier-directory.md`](../memory-bank/supplier-directory.md).
